const VIEW_KEYS = ["today", "24h", "7", "30", "history"];
const SOURCE_ORDER = ["codex", "cursor", "antigravity"];
const SOURCE_LABELS = {
    codex: "Codex",
    cursor: "Cursor",
    antigravity: "Antigravity"
};
export function mergeAgentPayloads(codex, cursor, antigravity) {
...
export function buildTotalStatusKind(statuses) {
    const values = SOURCE_ORDER.map((source) => statuses[source]);
    if (values.some((status) => status === "scanning"))
        return "scanning";
    if (values.some((status) => status === "error"))
        return "error";
    if (values.some((status) => status === "queued"))
        return "queued";
    if (values.some((status) => status === "cancelled"))
        return "cancelled";
    return "idle";
}
export function buildTotalStatusMessage(statuses, messages) {
    return SOURCE_ORDER.map((source) => {
        ...
        if (text.includes("已同步") || text.includes("已刷新"))
            return `${label} 已同步`;
        if (text.includes("暂无"))
            return `${label} 无数据`;
        return `${label}：${text}`;
...
function mergeTtfbRecords(sources) {
    const merged = [];
    for (const item of sources) {
...
function mergeFailureRecords(sources) {
    const merged = [];
    for (const item of sources) {
...
function mergePricingRules(payloads) {
    const seen = new Set();
    const merged = [];
...
function mergeViewsMap(sources) {
    const views = {};
...
function mergeView(left, right) {
    return {
        ...
        summary: mergeSummary(left.summary, right.summary),
...
function mergeCost(left, right, requests) {
...
function mergeTrendRows(left, right) {
...
function mergeDistributionRows(left, right) {
    const bucketMap = new Map();
    for (const row of [...left, ...right]) {
...
function mergeSessionRows(left, right) {
    const map = new Map();
    for (const row of [...left, ...right]) {
...
function mergeCostModelRows(left, right) {
    const map = new Map();
    for (const row of [...left, ...right]) {
...
function mergeCoverage(sources) {
    return sources.flatMap((item) => (item.payload.coverage ?? []).map((entry) => ({
        metric: entry.metric,
        source: item.source,
        status: entry.status
    })));
}
function resolveDataStatus(sources, recordCount) {
    if (recordCount > 0)
        return "ok";
    const statuses = sources.map((item) => item.payload.dataStatus).filter(Boolean);
    if (statuses.some((status) => status === "sync_failed"))
        return "sync_failed";
    if (statuses.some((status) => status === "parse_empty"))
        return "parse_empty";
    return "empty";
}
...
function parsePeakTokens(label) {
    const normalized = label.replace(/,/g, "");
    if (normalized.endsWith("亿"))
        return Number.parseFloat(normalized) * 100_000_000;
    if (normalized.endsWith("万"))
        return Number.parseFloat(normalized) * 10_000;
...
function parseLatencySeconds(label) {
    if (!label || label === "--")
...
function fmtInt(value) {
    ...
    if (abs >= 100_000_000)
        return `${formatSignificant(amount / 100_000_000)}亿`;
    if (abs >= 10_000)
        return `${formatSignificant(amount / 10_000)}万`;
    return amount.toLocaleString();
}
function comma(value) {
    return Math.trunc(value).toLocaleString();
}
//# sourceMappingURL=aggregatePayload.js.map