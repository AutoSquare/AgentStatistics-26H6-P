...
> **改前端后先构建 WebClient，再编译 WPF**  
> 宿主只读 `WebClient/dist/`，不读 `src/`。改 Vue/TS 后若跳过 `npm run build` 就 F5，界面（含总计页图表布局）不会更新。完整步骤见下文 [修改 WebClient 后、重新编译 WPF 之前必做](#修改-webclient-后重新编译-wpf-之前必做)。
...
### 修改 WebClient 后、重新编译 WPF 之前必做

WPF 宿主加载的是 `WebClient/dist/` 构建产物，不是 `src/` 源码。只改 Vue/TS 后直接 `dotnet build` 或 F5，界面不会更新。

每次改动 `WebClient/src/`、`WebClient/public/` 或前端依赖后，在编译 WPF **之前**按顺序执行：

```powershell
cd AgentStatistics\WebClient
npm run build
cd ..
dotnet build
```

首次克隆或 `package.json` 变更时，先执行 `npm install` 再 `npm run build`。

可用以下方式确认前端已写入宿主目录：

- `WebClient/dist/index.html` 与 `WebClient/dist/assets/` 的修改时间晚于本次前端改动；
- 重新运行后，总计页等 UI 变更已出现在 WebView2 仪表盘中。

若仅修改 `ASPy/` 或 C# 代码、未动 WebClient，可跳过 `npm run build`，直接 `dotnet build`。
...
当前版本已完成 AgentStatistics 的桌面应用骨架、Codex / Cursor / Antigravity 用量统计链路、跨 Agent 总计页（前端合并三源 payload）、Web 仪表盘、品牌图标接入、本地设置和快照持久化。通用文件分析仍保留为后续扩展入口，尚未接入实际数据源。
